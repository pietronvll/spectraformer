...
Let's start prepairing everything for the release. I have an idea to do a release on GitHub and then use Zenodo with their option to link the existing GitHub repository for FAIR-ness.

I have edited this logbook writings using *markdownlint* VSCode extension for style checking to get rid of all the warnings.

Things I would like to include in the release:

1. Installation guide
1. Readme (?)
1. `train_script.py`
1. `data/data_parser_script.py`
1. Spectroscopic data parsed with it
    * Buffer layer: `data/parsed_data_spatial/buffer+graphene/RUN3REC2_Buffer_20241011_1/5_5_1/RUN3REC2_Buffer_20241011_1_11x11_pt1.nc`
    * Monolayer graphene: `data/parsed_data_spatial/buffer+graphene/main/main_8x8_Mixed_8x8.nc`
    * SiC (included in the training): `data/parsed_data_spatial/SiC-high-f/main/main_20x10x3_DepthMapHR100x5p1s1acc.nc`
    * SiC (not included in the training): `data/parsed_data_spatial/SiC-high-f_not-in-dataset/main/main_10x10_Raman_map10x10_532nm_2100cm_5p_1acc_5s.nc`
1. `unmixing_v2.py`
1. logs for saved models
1. saved models themselves:
    * min23
    * min70
1. `requirements.txt` - **needs editing**

## Dec 12, 2025

Trained new models **min69** and **min70** - with the reduced mask, to see the effect of providing less information. Results are:

* if one shows only flat pure noise region - the model captures nothing at all (**min69**);
* if one shows a bit of SiC features - the model makes the job done nicely. Even providing less error in the shown region.
...
it is still negative. I have a hint - because of $$log(0<x<1)<<0$$.
...
2. Solve the issue with logging on Franklin.
3. Containerize the package for fast deployment.
...
1. Ad an asymmetric attention matrix masking any *inputs* from the masked region (but not outputs)
...
1. Working on GPU is *much* faster.
...
3. I need to implement a *testing* pipeline, with a proper data splitting.
...
*Problem:* Given a mixed RAMAN spectra of a Graphene - SiC (silicon carbide) sample, perform spectral unmixing.

*Data preprocessing:* A single data point is a SiC RAMAN spectra.
...
2. Compute frequency *and* counts embeddings and sum them $x \gets e_{f} + e_{c}$
3. Feed $x$ to a transformer and train it to recover the *unmasked* spectra with the MSE error.

Open questions:

* How to enforce the physical requirement of *positive* spectrum counts: that is, how to enforce that $y_{\rm mix} - y_{SiC} \geq 0$ ?
